...
subjects = ['102', '103', '104', '106', '107', '108', '110', '212', '213', '634',
...
for s in subjects:
    if not exists(join(work_dir, s)):
        makedirs(join(work_dir, s))
    ...
    outliers_censored = join(physics, s, 'session-0', 'resting-state', 'resting-state-0', 'endor1.feat', 'motion_outliers-censored.txt')
    if exists(outliers_censored):
        ...
        np.savetxt((join(data_dir, '{0}_confounds.txt'.format(s))), motion)
        confounds = join(physics, s, 'session-0', 'resting-state', 'resting-state-0', 'endor1.feat', 'mc', 'prefiltered_func_data_mcf.par')
    ...
    for p in np.arange(0.1, 1, 0.1):
        ...
        np.savetxt((join(work_dir, s, '{0}_network_correlation_matrix_{1}.csv'.format(s, p))), ntwk_corrmat_thresh, delimiter=',')
        regn_corrmat_thresh = bct.threshold_proportional(region_correlation_matrix, p, copy=True)
        np.savetxt((join(work_dir, s, '{0}_region_correlation_matrix_{1}.csv'.format(s, p))), regn_corrmat_thresh, delimiter=',')
        ...
        #global efficiency, because local=False by default
    ...
    np.savetxt((join(work_dir, s, '{0}_betweenness_centrality.csv'.format(s))), BC, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_node_degree.csv'.format(s))), DEG, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_positivenode_strength.csv'.format(s))), SPOS, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_negativenode_strength.csv'.format(s))), SNEG, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_total_positive_weight.csv'.format(s))), VPOS, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_total_negative_weight.csv'.format(s))), VNEG, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_nodal_efficiency.csv'.format(s))), LE, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_path_length.csv'.format(s))), PL, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_global_efficiency.csv'.format(s))), GL_EFF, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_eccentricity.csv'.format(s))), ECC, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_radius.csv'.format(s))), RAD, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_diameter.csv'.format(s))), DIA, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_community_index.csv'.format(s))), MOD, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_modularity.csv'.format(s))), Q, delimiter=',')
    np.savetxt((join(work_dir, s, '{0}_clustering_coefficient.csv'.format(s))), C, delimiter=',')
    ...
    plt.savefig((join(work_dir, s, '{0}_correlation_graph.png'.format(s))), dpi=400)
